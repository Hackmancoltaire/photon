...
class OracleAdaptor extends phObject {
	...
	*/
			...
			if (array_key_exists($this->connectionName,$GLOBALS["registeredDatabases"])) {
				if (array_key_exists("link",$GLOBALS["registeredDatabases"][$this->connectionName])) {
					...
					if (array_key_exists("args",$GLOBALS["registeredDatabases"][$this->connectionName])) {
						if (array_key_exists("serviceName",$GLOBALS["registeredDatabases"][$this->connectionName]["args"])) {
							// Proceed
							
							list($host,$port) = explode(":",$GLOBALS["registeredDatabases"][$this->connectionName]["host"]);
							
							if (is_null($port)) { $port = "1521"; }
							
							$GLOBALS["registeredDatabases"][$this->connectionName]["link"] = oci_pconnect(
								$GLOBALS["registeredDatabases"][$this->connectionName]["user"],
								$GLOBALS["registeredDatabases"][$this->connectionName]["password"],
								"(DESCRIPTION=(ADDRESS = (PROTOCOL = TCP)(HOST = $host)(PORT = $port))(CONNECT_DATA=(SERVICE_NAME=" . $GLOBALS["registeredDatabases"][$this->connectionName]["args"]["serviceName"] . ")))"
							);
						}
						else {
							// Fail, we need a service name
						}
					}
					else {
						// Fail, we need a service name
			...
			if (array_key_exists($this->connectionName,$GLOBALS["registeredDatabases"])) {
				if (array_key_exists("connectionCount",$GLOBALS["registeredDatabases"][$this->connectionName])) {
					...
					if (array_key_exists("link",$GLOBALS["registeredDatabases"][$this->connectionName]) && $GLOBALS["registeredDatabases"][$this->connectionName]["connectionCount"] < 0) {
						oci_close($GLOBALS["registeredDatabases"][$this->connectionName]["link"]);
	...
	function error($extra = null) {
		$error = oci_error($GLOBALS["registeredDatabases"][$this->connectionName]["link"]);
		
		if (!is_null($error["code"])) {	
			print $error["code"] . ": " . $error["message"] . (($extra != null) ? ": $extra":"") . "\n";
			$this->disconnect();
		}
	}
	...
	function initWithAlternateKey($key,$value) {
		...
		if ($key != NULL && $value != NULL) {
			...
			$result = oci_parse($GLOBALS["registeredDatabases"][$this->connectionName]["link"],$query);
			$executed = oci_execute($result);
			
			if (!$executed) { $this->error($query); }
			else {
				...
				if (!(is_null($row))) {				
					...
					if ($processKeys) {
						...
						foreach ($objectKeys as $key) {
							...
							else if ($testType == 2) { $this->$key = $row[$this->objectDBConnections[$key][0]]; } // String
							else if ($testType == 3) {
							...
							else if ($testType == 7) { $this->$key = strtotime($row[$this->objectDBConnections[$key][0]]); } // Date & Time (For Oracle: 06-NOV-2006 14:45:01)
							else if ($testType == 8) { $this->$key = $row[$this->objectDBConnections[$key][0]]->load(); } // Binary
